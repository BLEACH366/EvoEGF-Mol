...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    def __init__(self, num_blocks, num_layers, hidden_dim, n_heads=1, knn=32,
                 ...
                 cutoff_mode='radius', ew_net_type='r',          num_bond_classes=5, bond_net_type='lin',
    ...
    def forward(self, h, x, mask_ligand, mask_gen, batch, return_all=False, fix_x=False):
        ...
        for b_idx in range(self.num_blocks):
            ...
            for l_idx, layer in enumerate(self.base_block):
                h, x = layer(h, x, edge_type, edge_index, mask_gen, e_w=e_w, fix_x=fix_x)
            all_x.append(x)
            all_h.append(h)
        outputs = {'x': x, 'h': h}
